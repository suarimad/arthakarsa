...
// Menangkap Toast Session dari leave_add.php atau proses AJAX
...
// 1. Hak Melihat Semua Data (Admin, Superadmin, HR, Manager)
$can_view_all = in_array($role_name_session, ['admin', 'superadmin', 'hr', 'manager']);

// 2. Hak Persetujuan / Approval (Admin, Superadmin, HR, Manager)
$can_approve = in_array($role_name_session, ['admin', 'superadmin', 'hr', 'manager']);
...
<style>
    ...
    table.dataTable thead th { border-bottom: 1px solid #f3f4f6 !important; padding: 0.75rem 1rem !important; background-color: #f9fafb; }
                                                ...
                                                <td class="text-right">
                                                    <div class="flex items-center justify-end gap-1.5">
                                                        ...
                                                        <?php if ($can_approve && $status === 'pending'): ?>
                                                            <button onclick="openConfirmModal(<?= $lr['id'] ?>, 'approve')" class="p-2 bg-success/10 text-success rounded-xl text-xs font-semibold flex items-center justify-center hover:bg-success hover:text-white transition shadow-sm active:scale-95" title="Setujui">
                                                                <i data-lucide="check" class="w-3.5 h-3.5"></i>
                                                            </button>
                                                            <button onclick="openConfirmModal(<?= $lr['id'] ?>, 'reject')" class="p-2 bg-failed/10 text-failed rounded-xl text-xs font-semibold flex items-center justify-center hover:bg-failed hover:text-white transition shadow-sm active:scale-95" title="Tolak">
...
<script>
    ...
    // Menerima pesan toast dari PHP Session
