...
if (isset($_POST['ajax_action']) || isset($_GET['ajax_action'])) {
    ...
    try {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET' && $_GET['ajax_action'] === 'view') {
            ...
            // Hitung total posisi di departemen ini
            $stmt1 = $pdo->prepare("SELECT COUNT(id) as total_positions FROM positions WHERE department_id = ? AND tenant_id = ?");
            $stmt1->execute([$id, $tenant_id]);
            $res1 = $stmt1->fetch(PDO::FETCH_ASSOC);
            
            // Hitung total karyawan di departemen ini
            $stmt2 = $pdo->prepare("SELECT COUNT(u.id) as total_users FROM users u JOIN positions p ON u.position_id = p.id WHERE p.department_id = ? AND u.tenant_id = ?");
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['ajax_action'] === 'add') {
            ...
            $_SESSION['toast_msg'] = "Departemen berhasil ditambahkan!";
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['ajax_action'] === 'edit') {
            ...
            $_SESSION['toast_msg'] = "Departemen berhasil diperbarui!";
            $_SESSION['toast_type'] = "success";
            echo json_encode(['status' => 'success']);
            exit;
        }
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['ajax_action'] === 'delete') {
            $id = $_POST['department_id'];
            $stmt = $pdo->prepare("UPDATE departments SET deleted_at = CURRENT_TIMESTAMP WHERE id = ? AND tenant_id = ?");
            $stmt->execute([$id, $tenant_id]);
            
            $_SESSION['toast_msg'] = "Departemen berhasil dihapus!";
            $_SESSION['toast_type'] = "success";
            echo json_encode(['status' => 'success']);
            exit;
        }
...
// MENGAMBIL DATA DEPARTEMEN UNTUK RENDER HALAMAN
$stmt = $pdo->prepare("
    ...
    <main class="w-full bg-surface md:bg-transparent min-h-screen pb-24 md:pb-8 md:px-6">
        ...
        <div class="px-5 md:px-0 space-y-5 md:space-y-6 mt-2 relative z-0">
            ...
            <div class="flex justify-between items-center px-1">
                <div>
                    <h2 class="text-lg md:text-xl font-bold text-gray-800 tracking-tight">Manajemen Departemen</h2>
                    <p class="text-[11px] md:text-xs text-gray-500 mt-0.5">Kelola departemen dan divisi perusahaan</p>
                </div>
            ...
            <div class="relative z-0">
                ...
                <input type="text" id="searchInput" placeholder="Cari nama departemen..." class="w-full pl-11 pr-4 py-3 bg-surface md:bg-white border border-gray-200 md:border-gray-100 rounded-2xl text-sm focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary transition shadow-sm">
            </div>
                ...
                <div class="md:col-span-3 space-y-5 md:space-y-6">
                    ...
                    <section class="relative z-0">
                        <div class="flex justify-between items-center mb-3 px-1">
                            <h3 class="text-xs md:text-sm font-semibold text-gray-800">Daftar Departemen</h3>
                            <span class="text-[10px] font-medium bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full" id="departmentCount"><?= count($departments) ?> Departemen</span>
                        </div>
                        
                        <!-- Kontainer List Departemen -->
                        <div id="departmentListContainer" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4 relative z-0">
                            <?php foreach($departments as $dept): 
                                $safe_name = htmlspecialchars($dept['name'], ENT_QUOTES);
                            ?>
                                <div class="bg-surface border border-gray-100 rounded-xl p-4 shadow-sm flex flex-col gap-3 transition hover:border-gray-200 group department-card relative z-0" data-name="<?= strtolower($safe_name) ?>">
                                    ...
                                    <div class="flex items-center gap-3.5">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <p class="text-[11px] text-gray-500 font-medium truncate mt-0.5">Divisi</p>
                                        </div>
                                    </div>

                                    <!-- Aksi CRUD AJAX -->
                                    <div class="flex gap-1.5 mt-2">
                                        <button onclick="openCrud('view', <?= $dept['id'] ?>, '<?= $safe_name ?>')" class="flex-1 py-2 bg-gray-50 text-gray-600 rounded-lg text-[10px] font-semibold flex items-center justify-center gap-1 hover:bg-success/10 hover:text-success transition">
                                            <i data-lucide="eye" class="w-3.5 h-3.5"></i> Lihat
                                        </button>
                                        <button onclick="openCrud('edit', <?= $dept['id'] ?>, '<?= $safe_name ?>')" class="flex-1 py-2 bg-gray-50 text-gray-600 rounded-lg text-[10px] font-semibold flex items-center justify-center gap-1 hover:bg-primary/10 hover:text-primary transition">
                                            <i data-lucide="edit-3" class="w-3.5 h-3.5"></i> Edit
                                        </button>
                                        <button onclick="openCrud('delete', <?= $dept['id'] ?>, '<?= $safe_name ?>')" class="flex-1 py-2 bg-gray-50 text-gray-600 rounded-lg text-[10px] font-semibold flex items-center justify-center gap-1 hover:bg-failed/10 hover:text-failed transition">
                            ...
                            <?php if(empty($departments)): ?>
                                <div class="col-span-full bg-surface border border-dashed border-gray-200 rounded-2xl p-8 text-center">
                                    ...
                                    <h4 class="text-sm font-bold text-gray-800">Belum ada departemen</h4>
                                    <p class="text-xs text-gray-500 mt-1 max-w-xs mx-auto">Klik tombol tambah di atas untuk membuat departemen pertama Anda.</p>
                                </div>
                            <?php endif; ?>
                        </div>
                    </section>
...
<script>
    ...
    function openCrud(action, id = '', name = '') {
        // 1. Generate HTML berdasarkan aksi
        let html = '';
        if (action === 'add' || action === 'edit') {
            const title = action === 'add' ? 'Tambah Departemen' : 'Edit Departemen';
            html = `
                ...
                <form id="ajaxCrudForm">
                    ...
                    ${action === 'edit' ? `<input type="hidden" name="department_id" value="${id}">` : ''}
                    
                    <div class="space-y-4">
                        <div>
                            <label class="block text-[10px] font-semibold text-gray-600 mb-1.5 uppercase">Nama Departemen</label>
                            <input type="text" name="name" value="${name}" required placeholder="Misal: IT Development" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-xs focus:ring-1 focus:ring-primary focus:outline-none transition">
                        </div>
                    </div>
            ...
            html = `
                <div class="text-center">
                    ...
                    <h3 class="text-base font-bold text-gray-800 mb-2">Hapus Departemen?</h3>
                    <p class="text-xs text-gray-500 mb-6">Apakah Anda yakin ingin menghapus departemen <b>${name}</b>? Data ini hanya akan disembunyikan (soft delete).</p>
                    <form id="ajaxCrudForm">
            ...
            html = `
                <div class="text-center mb-6 mt-2 md:mt-0">
                    ...
                    <h3 class="text-base font-bold text-gray-800">Statistik Departemen</h3>
                ...
                <div id="viewStats" class="hidden space-y-3">
                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded-xl border border-gray-100">
                        <div class="flex items-center gap-3">
                            ...
                            <span class="text-xs font-semibold text-gray-700">Total Posisi / Jabatan</span>
                        </div>
            ...
            // Fetch View Data
            fetch(`?ajax_action=view&department_id=${id}`)
                .then(res => res.json())
                .then(data => {
