...
if (isset($_POST['ajax_action']) || isset($_GET['ajax_action'])) {
    ...
    try {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['ajax_action'] === 'add') {
            ...
            $stmt = $pdo->prepare("INSERT INTO shifts (tenant_id, name, start, end) VALUES (?, ?, ?, ?)");
            $stmt->execute([$tenant_id, $name, $start, $end]);
            
            $_SESSION['toast_msg'] = "Shift berhasil ditambahkan!";
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['ajax_action'] === 'edit') {
            ...
            $stmt = $pdo->prepare("UPDATE shifts SET name = ?, start = ?, end = ? WHERE id = ? AND tenant_id = ?");
            $stmt->execute([$name, $start, $end, $id, $tenant_id]);
            
            $_SESSION['toast_msg'] = "Shift berhasil diperbarui!";
            $_SESSION['toast_type'] = "success";
            echo json_encode(['status' => 'success']);
            exit;
        }
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['ajax_action'] === 'delete') {
            ...
            $_SESSION['toast_msg'] = "Shift berhasil dihapus!";
            $_SESSION['toast_type'] = "success";
            echo json_encode(['status' => 'success']);
            exit;
        }
...
// MENGAMBIL DATA SHIFT UNTUK RENDER HALAMAN
$stmt = $pdo->prepare("
    SELECT * FROM shifts 
    ...
    <main class="w-full bg-surface md:bg-transparent min-h-screen pb-24 md:pb-8 md:px-6">
        ...
        <div class="px-5 md:px-0 space-y-5 md:space-y-6 mt-2 relative z-0">
            ...
            <div class="relative z-0">
                ...
                <input type="text" id="searchInput" placeholder="Cari nama shift..." class="w-full pl-11 pr-4 py-3 bg-surface md:bg-white border border-gray-200 md:border-gray-100 rounded-2xl text-sm focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary transition shadow-sm">
            </div>
                ...
                <div class="md:col-span-3 space-y-5 md:space-y-6">
                    ...
                    <section class="relative z-0">
                        <div class="flex justify-between items-center mb-3 px-1">
                            <h3 class="text-xs md:text-sm font-semibold text-gray-800">Daftar Shift</h3>
                            <span class="text-[10px] font-medium bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full" id="shiftCount"><?= count($shifts) ?> Shift</span>
                        </div>
...
<script>
    ...
    function openCrud(action, id = '', name = '', start = '', end = '') {
        ...
        if (action === 'add' || action === 'edit') {
            ...
            html = `
                ...
                <form id="ajaxCrudForm">
                    ...
                    ${action === 'edit' ? `<input type="hidden" name="shift_id" value="${id}">` : ''}
            ...
            // Fetch View Data
            fetch(`?ajax_action=view&shift_id=${id}`)
                .then(res => res.json())
                .then(data => {
